 ...
 * Módulo 3  - Lista 2
 *
 * Atividade (questão 4): Projete um algoritmo concorrente para gerenciar
 * 			  uma fila de impressão. 
 *
 * 			  A thread que irá gerenciar a impressora deverá aguardar por 
 * 			  novas requisições de impressão, selecionar uma das requisições 
 * 			  pendentes, executar a impressão e voltar a aguardar por novas requisições.
 *			  As requisições devem ser atendidas na ordem em que foram recebidas.
 *
 * 			  As threads que gerarão as requisições de impressão deverão depositar as 
 * 			  requisições de impressão na fila de impressão. A fila de impressão terá 
 * 			  um tamanho limitado, então caso a thread encontre a fila cheia, ela deverá 
 * 			  ficar retida até conseguir depositar a sua requisição na fila, e só então 
 * 			  prosseguir com a sua execução normal.
 */
